...
dataPlotting1_plot2<-subset(dataPlotting1,Date=="2007-02-01"|Date=="2007-02-02")

#create the column "DateTime" with 
dataPlotting1_plot2$DateTime<-as.POSIXct(paste(dataPlotting1_plot2$Date,
                                               dataPlotting1_plot2$Time),
                                         format="%Y-%m-%d %H:%M:%S")

#create the PNG file with the requested attributes
png(file="plot2.png",width=480,height=480,units ="px")

#draw the graph with proper formating, labels...
with(dataPlotting1_plot2,plot(DateTime,Global_active_power,type="l",xlab="",
